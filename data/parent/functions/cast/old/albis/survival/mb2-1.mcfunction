summon area_effect_cloud ~ ~ ~ {Tags:["timer.albis.mb2-1"],Duration:80}
...
execute at @e[tag=aec.albis.mb2] as @e[team=!Friendly,nbt=!{Invulnerable:1b},distance=..1.5] store result score @s mobhealth run data get entity @s Health
execute at @e[tag=aec.albis.mb2] as @e[team=!Friendly,nbt=!{Invulnerable:1b},distance=..1.5] store result entity @s Health float 1 run scoreboard players remove @s mobhealth 7
